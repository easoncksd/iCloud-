...
class _BatchAccountPaused(Exception):
    pass


class _BatchAccountStopped(Exception):
    pass


_CREATE_CONTROL_FILE = RESULTS_DIR / "create_controls.json"
_account_control_lock = threading.RLock()
_account_controls = {}
...
def _save_create_controls_locked(path=None):
    target = Path(path or _CREATE_CONTROL_FILE)
    target.parent.mkdir(parents=True, exist_ok=True)
    payload = json.dumps(
        {"paused": sorted(_paused_account_ids)},
        ensure_ascii=False,
        indent=2,
    )
    tmp = target.with_suffix(target.suffix + ".tmp")
...
_paused_account_ids = _load_create_controls()


def _account_control(acc_id):
    acc_id = str(acc_id or "")
    with _account_control_lock:
        item = _account_controls.get(acc_id)
        if item is None:
            item = {"action": None, "event": threading.Event()}
            _account_controls[acc_id] = item
        return item


def _account_control_action(acc_id):
    return _account_control(acc_id)["action"]


def _set_account_control(acc_id, action):
    item = _account_control(acc_id)
    item["action"] = action
...
def _wait_account(acc_id, seconds):
    deadline = time.monotonic() + max(0.0, float(seconds or 0))
    event = _account_control(acc_id)["event"]
    while True:
        if _shutdown_event.is_set() or _account_control_action(acc_id) in ("pause", "stop"):
            return True
...
def _account_is_paused(acc_id):
    return str(acc_id or "") in _paused_account_ids
    ...
    while not _scheduler_stop_event.is_set() and not _shutdown_event.is_set():
        ...
        for i, account in enumerate(active_accounts):
            ...
            if _account_is_paused(acc_id):
                _emit_log("info", f"[{acc_name}] 已暂停创建，本轮跳过")
                continue
            ...
            halt_reason = {"value": None}

            def _scheduler_should_stop():
                if halt_reason["value"]:
                    return True
                if _scheduler_stop_event.is_set() or _shutdown_event.is_set():
                    halt_reason["value"] = "shutdown"
                    return True
                action = _account_control_action(acc_id)
                if action in ("pause", "stop"):
                    halt_reason["value"] = action
                    return True
                return False
            ...
            try:
                results = _account_mgr.create_aliases_for_account(
                    acc_id, target_count, "",
                    progress_callback=_ensure_pickup_for_created,
                    should_stop=_scheduler_should_stop,
            ...
            if halt_reason["value"] == "pause":
                _emit_log("info", f"[{acc_name}] 自动创建已暂停")
            elif halt_reason["value"] == "stop":
                _set_account_control(acc_id, None)
                _emit_log("info", f"[{acc_name}] 自动创建已停止")
...
_MAIL_AUTH_RECHECK_SECONDS = max(3600, int(os.environ.get("MAIL_AUTH_RECHECK_SECONDS", "21600")))


def _mail_account_paused(acc_id):
    account = getattr(_account_mgr, "accounts", {}).get(acc_id) or {}
    return bool(account.get("mail_sync_paused") or account.get("mail_status") == "auth_failed")


def _mail_retry_due(account):
    retry = account.get("mail_next_retry_at")
    try:
        if retry:
            return time.time() >= float(retry)
        last = datetime.fromisoformat(account.get("mail_last_checked") or "")
        return time.time() >= last.timestamp() + _MAIL_AUTH_RECHECK_SECONDS
    except (ValueError, TypeError, OverflowError):
        return False


def _initialize_mail_pauses():
    for account in _account_mgr.list_accounts():
        if _mail_account_paused(account["id"]) and not account.get("mail_next_retry_at"):
            _account_mgr.update_account(account["id"], mail_sync_paused=True,
...
def _apply_mail_watch_result(acc_id, ok, error_text="", verified=False):
    ...
    if ok:
        if _mail_account_paused(acc_id) and not verified:
            return "paused"
    ...
    if _mail_error_is_auth_failure(err):
        _account_mgr.update_account(
            ...
            mail_sync_paused=True,
            mail_next_retry_at=time.time() + _MAIL_AUTH_RECHECK_SECONDS,
            mail_last_error=err,
            mail_last_checked=now,
        )
        return "auth_failed"
    # A paused account stays paused after a network failure; defer the next probe.
    if _mail_account_paused(acc_id):
        if "收信已暂停" not in err:
...
_mail_probe_lock = threading.Lock()
_mail_probe_accounts = set()


def _check_account_mail(account, wait_timeout=0, fresh_seconds=None, force=False):
    acc_id = account.get("id")
    with _mail_probe_lock:
        if acc_id in _mail_probe_accounts:
            return "busy"
        _mail_probe_accounts.add(acc_id)
    try:
        return _check_account_mail_once(account, wait_timeout, fresh_seconds, force)
    finally:
        with _mail_probe_lock:
            _mail_probe_accounts.discard(acc_id)


def _check_account_mail_once(account, wait_timeout=0, fresh_seconds=None, force=False):
    ...
    try:
        ...
        if not lock.acquire(timeout=10):
            return "busy"
        try:
            result = _account_mgr.test_imap_connection(acc_id, allow_paused=True)
            return _apply_mail_watch_result(acc_id, bool(result.get("ok")), result.get("error") or "", verified=True)
        finally:
            lock.release()
    finally:
  ...
  <div class="page">
      ...
      <div class="panel">
        <div class="panel-header">
          ...
          <div>
            ...
            <button class="btn btn-primary btn-sm" id="btnExportSelected" onclick="exportSelectedPickupTxt()" data-i18n="emails.export_txt">导出已选 TXT</button>
        ...
        <div class="filter-bar">
          ...
          <div class="pager" id="aliasPager">
            <button class="btn btn-primary btn-sm" id="btnExportUnexported" onclick="exportUnexportedPickupTxt()" data-i18n="emails.export_unexported">一键导出未导出</button>
    ...
    <div id="view-settings" style="display:none">
      <div class="panel" style="padding:16px;margin-bottom:16px">
        <h3>代理与自动切换</h3>
        <div style="padding:12px;background:var(--green-soft);border-radius:8px;margin:10px 0">
          <strong>自动选择节点</strong>
          <button class="btn btn-sm" onclick="loadAutoProxyStatus()">刷新节点状态</button>
          <div id="autoProxyStatus" style="margin-top:8px;overflow-wrap:anywhere" aria-live="polite">正在读取自动代理状态…</div>
        </div>
        <p>下面是项目连接代理的入口。使用本机自动代理时，由后台选择出口节点；固定外部代理不会自动轮换。代理故障不回退直连。</p>
        <div class="toolbar">
          <label>网络方式 <select id="proxyMode"><option value="system">保持现有网络</option><option value="proxy">固定代理</option><option value="direct">直接连接</option></select></label>
          <label>协议 <select id="proxyProtocol"><option value="http">HTTP CONNECT</option><option value="socks5h">SOCKS5（代理解析域名）</option></select></label>
          <label>主机 <input type="text" id="proxyHost" placeholder="IP 或域名" autocomplete="off"></label>
          <label>端口 <input id="proxyPort" type="number" min="1" max="65535" value="8080"></label>
          <label>用户名 <input type="text" id="proxyUsername" autocomplete="off"></label>
          <label>密码 <input id="proxyPassword" type="password" autocomplete="new-password" placeholder="留空保留已保存密码"></label>
          <label><input id="proxyClearPassword" type="checkbox">清除已保存密码</label>
          <button class="btn btn-sm" onclick="testNetworkProxy()">测试未保存配置</button>
          <button class="btn btn-sm" onclick="saveNetworkProxy()">保存网络配置</button>
        </div>
        <p id="proxyStatus">正在读取配置…</p>
        <p>测试仅检查 HTTPS 和 IMAP 网络连接，不登录账号、不创建邮箱。HTTP 代理需支持 CONNECT 到 993 端口。保存后新请求使用新配置；已有 IMAP 连接在下次操作时重连。</p>
        <hr>
        <h4>自定义订阅地址</h4>
        <p>可保存你的机场 HTTPS 订阅地址。地址和 token 不会回显或写入日志；保存后由服务器上的代理同步流程读取。</p>
        <div class="toolbar">
          <label style="min-width:320px;flex:1">订阅地址 <input type="url" id="proxySubscriptionUrl" placeholder="https://…" autocomplete="off" style="width:100%"></label>
          <button class="btn btn-sm" onclick="saveProxySubscription()">保存订阅地址</button>
          <button class="btn btn-sm" onclick="clearProxySubscription()">清除订阅地址</button>
        </div>
        <p id="proxySubscriptionStatus">正在读取订阅配置…</p>
      </div>
      <div class="panel" style="padding:16px;margin-bottom:16px">
        <h3>创建保护</h3>
        <p>临时限流固定冷却 30 分钟；认证、额度或不明确的失败停止创建。收信异常账号禁止新建。下列额度是项目保护参数，不是 Apple 安全额度。</p>
        <div class="toolbar">
          <label>全局并发 <input id="guardConcurrency" type="number" min="1" max="10" value="3"></label>
          <label>每账号每日尝试上限 <input id="guardDaily" type="number" min="1" max="750" value="50"></label>
          <button class="btn btn-sm" onclick="saveCreationGuard()">保存保护设置</button>
          <button class="btn btn-sm" onclick="loadCreationGuard()">刷新保护记录</button>
          <button class="btn btn-sm" onclick="releaseCreationGuard()">清除多账号异常提醒</button>
        </div>
        <p>每日次数按北京时间零点重置，失败尝试也计数；10 分钟内 3 个账号出现同类异常时仅提醒，不暂停其他账号。每个账号独立冷却、独立停止。解除不会绕过 30 分钟冷却。</p>
        <div id="creationGuardStatus" style="overflow-wrap:anywhere"></div>
      </div>
      <div class="panel">
        ...
        <div class="settings-section">
          ...
          <p data-i18n="settings.create_desc">不同主账号受创建保护中的全局并发限制；同一账号仍逐个创建，触发 Apple 临时限制时每次等待 30 分钟后自动续建。创建过程中可勾选账号暂停或停止：暂停会保留剩余数量，停止会取消剩余数量。</p>          <div id="batchAccCount" class="hint">0 个可用账号</div>
          ...
          <div class="toolbar">
            ...
            <button class="btn btn-outline" id="btnBatchPause" onclick="controlBatchCreate('pause')" data-i18n="settings.pause" disabled>暂停</button>
            <button class="btn btn-outline" id="btnBatchStop" onclick="controlBatchCreate('stop')" data-i18n="settings.stop" disabled>停止</button>
          </div>
...
function showTab(tab){
  ...
  if(curTab==='settings'){renderBatchPanel();renderLogs();loadAutoProxyStatus();}
...
function renderSidebar(){
  ...
  if(E('sideStatReady'))E('sideStatReady').textContent=accounts.filter(function(a){return a.has_app_password&&!a.mail_sync_paused&&a.mail_status!=='auth_failed'}).length;
  var bs=E('btnSched');
...
function renderDashboard(){
  ...
  c.innerHTML=accounts.map(function(a){
    ...
    var expired=a.status!=='active';
    var actions='<div class="acc-actions">'+(expired?('<button class="btn btn-primary btn-xs" onclick="showReimportModal(\''+escAttr(a.id)+'\')">'+t('action.reimport')+'</button><button class="btn btn-outline btn-xs" onclick="handlePrimaryAction(\''+escAttr(a.id)+'\')">'+t('action.create_alias')+'</button>'):('<button class="btn btn-primary btn-xs" onclick="handlePrimaryAction(\''+escAttr(a.id)+'\')">'+t('action.create_alias')+'</button>'))+'<button class="btn btn-outline btn-xs" onclick="validateAccount(\''+escAttr(a.id)+'\')">'+t('action.check_login')+'</button><button class="btn btn-outline btn-xs" onclick="showAppPwdModal(\''+escAttr(a.id)+'\')">'+t('action.set_mail')+'</button><button class="btn btn-outline btn-xs" onclick="removeAccount(\''+escAttr(a.id)+'\')">'+t('action.delete')+'</button></div>';
    if(a.mail_sync_paused||a.mail_status==='auth_failed')actions=actions.replace('</div>','<button class="btn btn-outline btn-xs" onclick="resumeMail(this,\''+escAttr(a.id)+'\')">'+t('mail.resume')+'</button></div>');
    return '<div class="acc-card'+(jobBusy?' is-busy':'')+(expired?' is-expired':'')+'"><div class="acc-top"><div><div class="acc-title">'+esc(a.name||t('name.unnamed'))+'</div><div class="acc-email">'+esc(email)+'</div></div><span class="status-badge '+stCls+'">'+esc(stText.substring(0,42))+'</span></div><div class="acc-usage"><div class="progress-head"><span>'+t('usage.capacity')+'</span><span>'+used+' / '+limit+'</span></div><div class="progress-bar"><div class="fill ok" style="width:'+pct+'%"></div></div></div><div class="acc-stats"><div>'+esc(mailReady)+'</div></div>'+jobHtml+actions+'</div>';  }).join('');
}
async function resumeMail(button,id){button.disabled=true;button.textContent=t('mail.checking');try{var d=await api('/api/accounts/'+encodeURIComponent(id)+'/mail-resume',{method:'POST'});toast(d.ok?t('mail.restored'):(d.error||t('error.unknown')),!d.ok);await refreshAll();}finally{button.disabled=false;button.textContent=t('mail.resume');}}
...
var pickupExportBusy=false;
async function exportSelectedPickupTxt(){if(pickupExportBusy)return;var selected=emails.filter(function(e){return !e.exported&&pickupSelected[String(e.email||'').toLowerCase()]}).map(function(e){return e.email});if(!selected.length){toast(t('export.need_select'),true);return}await exportPickupTxt({emails:selected},selected);}
async function exportUnexportedPickupTxt(){if(pickupExportBusy)return;await exportPickupTxt({unexported:true,account_id:E('aliasFilter').value},[]);}
async function exportPickupTxt(payload,selected){if(pickupExportBusy)return;pickupExportBusy=true;var buttons=[E('btnExportSelected'),E('btnExportUnexported')];buttons.forEach(function(b){if(b)b.disabled=true});try{
var d=await apiSlow('/api/pickup-links/export',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!d.ok){toast(t('export.fail',{err:d.error||t('error.unknown')}),true);return}if(!(d.lines||[]).length){toast(t(payload.unexported?'export.none_unexported':'export.already'),true);await refreshEmails();renderAliasTable();return}var b=new Blob(['\uFEFF'+d.lines.join('\n')],{type:'text/plain;charset=utf-8'}),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='icloud_mail_pickup_links_'+new Date().toISOString().slice(0,10)+'.txt';a.click();setTimeout(function(){URL.revokeObjectURL(a.href)},1000);selected.forEach(function(email){delete pickupSelected[String(email).toLowerCase()]});await refreshEmails();renderAliasTable();toast(t('export.done',{n:d.count}));
}catch(err){toast(t('export.fail',{err:err.message||t('error.unknown')}),true)}finally{pickupExportBusy=false;buttons.forEach(function(b){if(b)b.disabled=false});}}
...
function showAddAccountModal(){closeAddAccModal();var h='<div class="modal-overlay" id="addAccModal" onclick="if(event.target===this)closeAddAccModal()"><div class="modal-box"><h3>'+t('action.add_account')+'</h3><p>'+t('add.help_before')+' <a href="https://chromewebstore.google.com/detail/cookie-editor/hlkenndednhfkekhgcdicdfddnkalmdm" target="_blank" rel="noopener noreferrer">Cookie Editor</a>'+t('add.help_mid')+'<br>'+t('add.help_json')+'<code>{"name1":"value1"}</code> <a href="https://chromewebstore.google.com/detail/cookie-editor/hlkenndednhfkekhgcdicdfddnkalmdm" target="_blank" rel="noopener noreferrer">'+t('add.download')+'</a></p><label class="hint">'+t('add.region')+'</label><select id="accHostInput"><option value="icloud.com" selected>'+t('add.region_intl')+'</option><option value="icloud.com.cn">'+t('add.region_cn')+'</option></select><input type="text" id="accNameInput" placeholder="'+escAttr(t('add.name_ph'))+'"><textarea id="cookieInput" placeholder="'+escAttr(t('add.cookie_ph'))+'"></textarea><div class="modal-actions"><button class="btn btn-outline" onclick="closeAddAccModal()">'+t('action.cancel')+'</button><button class="btn btn-primary" id="btnAddAccount" onclick="addAccount()">'+t('action.add_account')+'</button></div><div class="modal-msg" id="addAccMsg"></div></div></div>';document.body.insertAdjacentHTML('beforeend',h);}
function closeAddAccModal(){window._reimportAccId=null;var m=E('addAccModal');if(m)m.remove()}
function showReimportModal(accId){closeAddAccModal();var acc=accounts.find(function(x){return x.id===accId})||{};window._reimportAccId=accId;var email=acc.real_email||acc.name||accId;var host=acc.host||'icloud.com';var intlSel=host==='icloud.com.cn'?'':' selected';var cnSel=host==='icloud.com.cn'?' selected':'';var h='<div class="modal-overlay" id="addAccModal" onclick="if(event.target===this)closeAddAccModal()"><div class="modal-box"><h3>'+t('reimport.title')+'</h3><p>'+t('reimport.help',{email:esc(email)})+'<br>'+t('add.help_before')+' <a href="https://chromewebstore.google.com/detail/cookie-editor/hlkenndednhfkekhgcdicdfddnkalmdm" target="_blank" rel="noopener noreferrer">Cookie Editor</a>'+t('add.help_mid')+'</p><label class="hint">'+t('add.region')+'</label><select id="accHostInput"><option value="icloud.com"'+intlSel+'>'+t('add.region_intl')+'</option><option value="icloud.com.cn"'+cnSel+'>'+t('add.region_cn')+'</option></select><textarea id="cookieInput" placeholder="'+escAttr(t('add.cookie_ph'))+'"></textarea><div class="modal-actions"><button class="btn btn-outline" onclick="closeAddAccModal()">'+t('action.cancel')+'</button><button class="btn btn-primary" id="btnAddAccount" onclick="submitReimport()">'+t('reimport.submit')+'</button></div><div class="modal-msg" id="addAccMsg"></div></div></div>';document.body.insertAdjacentHTML('beforeend',h);var box=E('cookieInput');if(box)box.focus();}
async function submitReimport(){var accId=window._reimportAccId;if(!accId)return;var cookies=E('cookieInput').value.trim();if(!cookies){E('addAccMsg').innerHTML='<span style="color:var(--red)">'+t('add.need_cookie')+'</span>';return}var btn=E('btnAddAccount');btn.disabled=true;btn.textContent=t('add.checking');var d=await api('/api/accounts/'+encodeURIComponent(accId)+'/reimport',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cookie_input:cookies,host:(E('accHostInput')&&E('accHostInput').value)||'icloud.com'})});btn.disabled=false;btn.textContent=t('reimport.submit');if(d.ok){E('addAccMsg').innerHTML='<span style="color:var(--green)">'+t('reimport.ok',{email:esc(d.real_email||'')})+'</span>';setTimeout(closeAddAccModal,1200);refreshAll();}else{E('addAccMsg').innerHTML='<span style="color:var(--red)">'+esc(d.error||t('status.login_expired'))+'</span>';}}
...
def api_reimport_account(acc_id):
    ...
    if requested_host in ("icloud.com", "icloud.com.cn"):
        ...
        host = AccountManager.detect_icloud_host(cookie_input)
    try:
        account = _account_mgr.reimport_account(acc_id, cookie_input, host=host)
        _emit_log(
            "info",
            f"重新导入账号: {account.get('name', '')} ({account.get('real_email', '?')})",
        )
        return jsonify({
            "ok": True,
            "id": account["id"],
...
_FINISHED_BATCH_ACCOUNT_STATUSES = ("completed", "partial", "failed", "limited", "stopped")
_PAUSED_BATCH_ACCOUNT_STATUS = "paused"
...
def api_create_for_account(acc_id):
    ...
    try:
        ...
        results = _account_mgr.create_aliases_for_account(
            acc_id, count, label,
            progress_callback=_ensure_pickup_for_created,
            should_stop=_manual_should_stop,
            wait=lambda seconds: _wait_account(acc_id, seconds),
        )
...
def api_network_proxy():
    ...
    try:
        if request.method == "GET":
            return jsonify(ok=True, config=network_proxy.public_config())
        data = request.get_json(silent=True) or {}
        if not isinstance(data, dict):
            return jsonify(ok=False, error="配置格式错误"), 400
        if data.get("action") == "test":
            config = network_proxy.validate(data, network_proxy.load_config())
            result = network_proxy.probe(config)
            return jsonify(ok=all(x['ok'] for x in result.values()), results=result)
...
def api_auto_proxy_status():
    from proxy_status import read_status
    return jsonify(ok=True, status=read_status())


@app.route("/api/proxy-subscription", methods=["GET", "POST"])
def api_proxy_subscription():
    import subscription_manager
    try:
        if request.method == "GET":
            return jsonify(ok=True, subscription=subscription_manager.status())
        data = request.get_json(silent=True) or {}
        if data.get("action") == "clear":
            return jsonify(ok=True, subscription=subscription_manager.clear())
        if data.get("action") != "save":
            return jsonify(ok=False, error="无效操作"), 400
        result = subscription_manager.save(data.get("url", ""))
...
def _sync_creation_journal(job, acc_id):
    """Repair a batch progress write interrupted after durable creation completion."""
    guard = getattr(_account_mgr, "creation_guard", None)
    if guard is None or not job.get("id"):
        return
    ...
    with _batch_lock:
        ...
        job["total_created"] = sum(int(e.get("created", 0)) for e in job["accounts"].values())
...
def _create_account_with_cooldown(job, acc_id, count, label, name):
    ...
    record_progress.task_id = job.get("id")
    halt_reason = {"value": None}
    ...
    def _raise_if_halted():
        reason = halt_reason["value"] or _account_control_action(acc_id)
        if _shutdown_event.is_set() or reason == "shutdown":
            raise _BatchInterrupted()
        if reason == "pause":
            raise _BatchAccountPaused()
        if reason == "stop":
            raise _BatchAccountStopped()
...
def _run_batch_account(job, acc_id, count, label):
    ...
    with _batch_lock:
        ...
        if entry.get("status") == "paused" or action == "pause":
            skip = "pause"
        elif entry.get("status") == "stopped" or action == "stop":
            skip = "stop"
        else:
            entry["status"] = "running"
    ...
    try:
        ...
        with _batch_lock:
            ...
            job["updated_at"] = datetime.now(_BJ_TZ).isoformat()
            _save_batch_state_locked()
        _emit_log("info", f"[{name}] 已暂停，已创建 {created} 个，剩余已保留")
        return job.get("completed_accounts", 0)
        ...
        with _batch_lock:
            ...
            _save_batch_state_locked()
        _set_account_control(acc_id, None)
        _emit_log("info", f"[{name}] 已停止，已创建 {created} 个，剩余已取消")
...
def _run_batch_job(job_id):
    ...
    _emit_log(
        "info", f"批量任务启动: {total_accounts} 个账号 x{count}，冷却账号独立等待，实际并发遵循保护设置"
    )
    ...
    try:
        ...
        with _batch_lock:
            paused_left = any(
                (entry or {}).get("status") == "paused"
                for entry in (job.get("accounts") or {}).values()
            )
            active_left = any(
                (entry or {}).get("status") in _ACTIVE_BATCH_ACCOUNT_STATUSES
                for entry in (job.get("accounts") or {}).values()
            )
            total_created = job.get("total_created", 0) or 0
            total_errors = job.get("total_errors", 0) or 0
            if paused_left and not active_left:
                job["status"] = "paused"
        ...
        _emit_log(
            ...
            (
                ...
                if final_status == "paused"
                else f"批量任务完成: {total_created} 成功 / {total_errors} 失败"
            ),
        )
    except _BatchInterrupted:
        with _batch_lock:
        ...
        with _batch_lock:
            ...
            if _batch_active_id == job_id and job.get("status") not in ("queued", "running", "paused"):                _batch_active_id = None
            _save_batch_state_locked()
...
def api_create_batch():
    ...
    with _manual_create_lock:
        ...
        with _batch_lock:
            ...
            start_runner = True
            if active and active.get("status") in ("queued", "running", "paused"):
                overlap = []
                new_ids = []
                resumed_ids = []
...
def _sync_job_status_locked(job):
    global _batch_active_id
    accounts = (job.get("accounts") or {}).values()
    has_active = any(
        (entry or {}).get("status") in _ACTIVE_BATCH_ACCOUNT_STATUSES
...
def _mark_batch_account(acc_id, action):
    global _batch_active_id
    with _batch_lock:
        if not _batch_active_id:
            return None
        ...
        status = entry.get("status")
        if action == "pause":
            if status not in _ACTIVE_BATCH_ACCOUNT_STATUSES and status != "paused":
                return None
            if status == "queued":
                entry["status"] = "paused"
                entry["error"] = "已暂停，剩余数量已保留"
                entry["retry_at"] = None
        ...
        if action == "stop":
            ...
            if status in ("queued", "paused"):
                ...
                entry["error"] = "已停止，剩余数量已取消"
                entry["retry_at"] = None
                entry["finished_at"] = datetime.now(_BJ_TZ).isoformat()
                job["completed_accounts"] = sum(
                    1 for item in job["accounts"].values() if item.get("finished_at")
                )
                _sync_job_status_locked(job)
                _save_batch_state_locked()
            return entry
...
def _pause_account_create(acc_id):
    acc_id = str(acc_id)
    in_progress = _account_create_in_progress(acc_id)
    marked = _mark_batch_account(acc_id, "pause")
    if not in_progress and marked is None:
        return False
    with _account_control_lock:
        _paused_account_ids.add(acc_id)
        _save_create_controls_locked()
    _set_account_control(acc_id, "pause")
    return True


def _stop_account_create(acc_id):
    acc_id = str(acc_id)
    in_progress = _account_create_in_progress(acc_id)
    paused = acc_id in _paused_account_ids
    marked = _mark_batch_account(acc_id, "stop")
    if not in_progress and marked is None and not paused:
        return False
    with _account_control_lock:
        _paused_account_ids.discard(acc_id)
        _save_create_controls_locked()
    _set_account_control(acc_id, "stop")
    if marked is None and not in_progress:
        _set_account_control(acc_id, None)
    return True
...
def _resume_account_create(acc_id):
    ...
    with _account_control_lock:
        ...
        _save_create_controls_locked()
    _set_account_control(acc_id, None)
    marked = _mark_batch_account(acc_id, "resume")
    start_runner = False
    job_id = None
    with _batch_lock:
        job_id = _batch_active_id
        job = _batch_jobs.get(job_id) if job_id else None
        if marked is not None and job and job.get("status") in ("queued", "running", "paused"):
            if job.get("status") == "paused":
                job["status"] = "queued"
                _save_batch_state_locked()
            start_runner = job_id not in _batch_runner_jobs
    if start_runner and job_id:
        threading.Thread(target=_run_batch_job, args=(job_id,), daemon=True).start()
    return True


@app.route("/api/create-control", methods=["POST"])
def api_create_control():
    ...
    for acc_id in account_ids:
        if action == "pause":
            ok = _pause_account_create(acc_id)
        elif action == "stop":
            ok = _stop_account_create(acc_id)
        else:
            ok = _resume_account_create(acc_id)
        (affected if ok else skipped).append(acc_id)
    if not affected:
        return jsonify({
            "ok": False,
            "error": "所选账号没有可停止或暂停的创建任务",
            "skipped": skipped,
            "job": _batch_job_snapshot(_batch_active_id) if _batch_active_id else None,
        }), 409
    names = []
    for acc_id in affected:
        account = _account_mgr.get_account(acc_id) or {}
        names.append(account.get("name") or acc_id)
    label = "、".join(names[:4]) + (" 等" if len(names) > 4 else "")
    if action == "pause":
        _emit_log("info", f"已暂停创建: {label}")
    elif action == "stop":
        _emit_log("info", f"已停止创建: {label}")
    else:
        _emit_log("info", f"已继续创建: {label}")
    return jsonify({
        "ok": True,
        "action": action,
        "affected": affected,
...
def api_export_pickup_links():
    ...
    if data.get("unexported") is True:
        account_id = str(data.get("account_id") or "")
        live_accounts = _account_mgr.accounts
        if account_id != "all" and account_id not in live_accounts:
            return jsonify({"ok": False, "error": "账号不存在，请刷新列表"}), 400
        history = _export_store.status_map(by_email)
        requested = [
            email for email, item in by_email.items()
            if item["account_id"] in live_accounts
            and (account_id == "all" or item["account_id"] == account_id)
            and email not in history
        ]
    else:
...
def _refresh_pickup_body(account_id, msg_id):
    ...
    try:
        ...
        _apply_mail_watch_result(account_id, False, str(e))
...
def pickup_messages(token):
    ...
    public_error = ("收信已暂停，请联系管理员验证恢复" if paused else "邮件同步暂时失败") if (paused or refresh_error) and not cached else None
    warning = ("收信已暂停，当前显示缓存内容" if paused else "邮件同步暂时失败，当前显示缓存内容") if (paused or refresh_error) and cached else None
...
def pickup_message(token, msg_id):
    ...
    if _mail_account_paused(account_id):
        return jsonify({"ready": False, "paused": True, "error": "收信已暂停，请联系管理员验证恢复"}), 503, {"Cache-Control": "no-store"}
    with _pickup_refresh_lock:
        if account_id in _mail_watch_hold_accounts:
...
_mail_resume_lock = threading.Lock()
_mail_resume_inflight = set()
_mail_resume_last = {}


@app.route("/api/accounts/<acc_id>/mail-resume", methods=["POST"])
def api_resume_mail(acc_id):
    account = _account_mgr.get_account(acc_id)
    if not account:
        return jsonify({"ok": False, "error": "账号不存在"}), 404
    if not account.get("app_password"):
        return jsonify({"ok": False, "error": "请先设置收信密码"}), 400
    with _mail_resume_lock:
        if acc_id in _mail_resume_inflight or time.monotonic() - _mail_resume_last.get(acc_id, -1000) < 60:
            return jsonify({"ok": False, "error": "请等待一分钟后再验证"}), 429
        _mail_resume_inflight.add(acc_id)
        _mail_resume_last[acc_id] = time.monotonic()
    try:
        status = _check_account_mail(account, wait_timeout=10, force=True)
        if status == "ok":
            _emit_log("info", f"收信验证成功，已恢复同步 [{acc_id}]")
            _schedule_pickup_account_refresh(acc_id)
            return jsonify({"ok": True})
        message = "收信任务正忙，请稍后再试" if status == "busy" else "收信验证未通过，暂停账号继续等待延迟复查；请检查官网邮件和收信密码"
        return jsonify({"ok": False, "error": message}), 409
    finally:
        with _mail_resume_lock:
            _mail_resume_inflight.discard(acc_id)
    ...
    def _shutdown(sig,frame):
        ...
        def finish_shutdown():
            ...
            while time.monotonic() < deadline:
                with _batch_lock:
                    runners = bool(_batch_runner_jobs)
                guard = _account_mgr.creation_guard
                with guard.lock:
                    active = bool(guard.active or guard.queued)
                if not active and not runners:
                    break
                time.sleep(0.1)
            _pickup_executor.shutdown(wait=False, cancel_futures=True)
            os._exit(0)
        threading.Thread(target=finish_shutdown, daemon=True, name="graceful-shutdown").start()
