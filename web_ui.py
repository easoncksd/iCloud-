...
@media (prefers-reduced-motion:reduce){
  ...
  .btn,.acc-card,.work-stat,.nav-item,.copy-toast,.progress-bar .fill,.email-table td,.segmented button,.lang-switch button{transition:none}
    ...
    <div id="view-settings" style="display:none">
      <div class="panel">
        ...
        <div class="settings-section">
          ...
          <p data-i18n="settings.create_desc">不同主账号最多 5 个并行；同一账号仍逐个创建，触发 Apple 临时限制时每次等待 1 分钟后自动续建。</p>
...
async function validateAccount(accId){var d=await api('/api/accounts/'+encodeURIComponent(accId)+'/validate',{method:'POST'});if(d.ok)toast(t('status.login_ok_email',{email:d.real_email}));else toast(t('status.login_expired'),true);refreshAll();}
