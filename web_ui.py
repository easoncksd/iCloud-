        ...
        <div class="panel-header">
          ...
          <div class="inbox-tools">
            ...
            <div class="alias-search" id="aliasSearchWrap">
              <input type="text" id="aliasSearchInput" placeholder="搜索隐私邮箱查件..." title="搜索已有隐私邮箱，查询这个地址的邮件" autocomplete="off" spellcheck="false">
              <button type="button" class="alias-search-clear" id="btnAliasSearchClear" hidden onclick="clearAliasSearch()" title="清除查询，查看全部收件">&times;</button>
...
async function searchAliasMail(force){var input=E('aliasSearchInput');var alias=resolveAliasQuery(input?input.value:'');if(!alias){toast('请输入隐私邮箱地址',true);return}if(input)input.value=alias;updateAliasSearchClear();hideAliasSuggest();var accId=E('inboxAccount')?E('inboxAccount').value:'';if(!accId){var hit=emails.find(function(e){return String(e.email||'').toLowerCase()===alias.toLowerCase();});if(hit){accId=hit.account_id;if(E('inboxAccount'))E('inboxAccount').value=accId;}}if(!accId){toast('请先选择账号',true);return}var acc=accounts.find(function(a){return a.id===accId;});if(acc&&!acc.has_app_password){renderInboxSetupHint();return}var seq=beginInboxRequest();E('inboxMsgs').innerHTML='<div class="empty"><div class="icon"></div>正在查询 '+esc(alias)+' ...</div>';var limit=parseInt(E('inboxLimit')&&E('inboxLimit').value,10)||20;var path='/api/accounts/'+encodeURIComponent(accId)+'/mail/'+encodeURIComponent(alias)+'?limit='+limit+(force?'&force=1':'');var d=await api(path,{timeout:120000});if(!inboxRequestCurrent(seq,accId))return;if(d.error){E('inboxMsgs').innerHTML='<div class="empty"><div class="icon"></div>'+esc(d.error)+'</div>'}else{var msgs=d.emails||[];renderInboxMsgs(msgs,msgs.length?('仅显示 '+alias+'（'+(d.count||msgs.length)+' 封）'):(alias+' 暂无收件'),accId)}finishInboxRequest(seq);}
async function checkAliasMail(){if(E('aliasSearchInput'))E('aliasSearchInput').value='';updateAliasSearchClear();hideAliasSuggest();var accId=E('inboxAccount').value;if(!accId){toast('请先选择账号',true);return}var seq=beginInboxRequest();E('inboxMsgs').innerHTML='<div class="empty"><div class="icon"></div>正在检查各邮箱的收件...</div>';var d=await api('/api/accounts/'+encodeURIComponent(accId)+'/alias-mail',{timeout:120000});if(!inboxRequestCurrent(seq,accId))return;if(d.error){E('inboxMsgs').innerHTML='<div class="empty"><div class="icon"></div>'+esc(d.error||'查询失败')+'</div>';finishInboxRequest(seq);return}var byAlias=d.by_alias||{},total=0,aliasKeys=Object.keys(byAlias),h='';if(!aliasKeys.length){E('inboxMsgs').innerHTML='<div class="empty"><div class="icon"></div>所有隐私邮箱暂无收件</div>';finishInboxRequest(seq);return}aliasKeys.forEach(function(alias){var msgs=sortInboxNewest(byAlias[alias]||[]);total+=msgs.length;h+='<div style="padding:8px 14px;border-bottom:1px solid var(--line);font-weight:600">'+esc(alias)+' ('+msgs.length+' 封)</div>';msgs.forEach(function(m){h+='<div style="padding:6px 20px;border-bottom:1px solid var(--line);font-size:12px;display:flex;justify-content:space-between;flex-wrap:wrap;gap:4px"><span><strong>'+esc(m.subject||'(无主题)')+'</strong></span><span style="color:var(--muted)">'+esc(m.from||'').substring(0,30)+'</span><span class="hint">'+formatBeijingTime(m.date)+'</span></div>';});});E('inboxMsgs').innerHTML='<div class="hint" style="padding:8px 14px;border-bottom:1px solid var(--line)">共 '+aliasKeys.length+' 个邮箱收到 '+total+' 封邮件</div>'+h;finishInboxRequest(seq);}
...
function renderInboxMsgs(msgs,title,accountId){msgs=sortInboxNewest(msgs);_inboxRenderedAccount=accountId||E('inboxAccount').value;if(!msgs.length){E('inboxMsgs').innerHTML='<div class="empty"><div class="icon"></div>'+esc(title||'收件箱为空')+'</div>';return}var h='<div class="hint" style="padding:8px 16px;border-bottom:1px solid var(--line)">'+esc(title)+'</div>';msgs.forEach(function(m,i){var mid=m.id||'m'+i;h+='<div class="email-item" style="border-bottom:1px solid var(--line);cursor:pointer" onclick="toggleEmail(\''+escAttr(mid)+'\',\''+escAttr(m.id||'')+'\',\''+escAttr(_inboxRenderedAccount)+'\')"><div style="padding:12px 16px;display:flex;justify-content:space-between;align-items:flex-start;gap:12px"><div style="flex:1;min-width:0"><div style="font-weight:600;margin-bottom:4px">'+esc(m.subject||'(无主题)')+'</div><div style="font-size:12px;color:var(--muted)">'+esc(m.from||'')+'</div><div class="hint" style="margin-top:2px">To: '+esc((m.to||'').substring(0,50))+'</div></div><div class="hint" style="white-space:nowrap">'+formatBeijingTime(m.date)+'</div></div><div id="'+escAttr(mid)+'_body" style="display:none;padding:0 16px 16px;line-height:1.7;color:var(--muted);white-space:pre-wrap;word-break:break-word;max-height:400px;overflow-y:auto;border-top:1px solid var(--line)"></div></div>'});E('inboxMsgs').innerHTML=h;}
