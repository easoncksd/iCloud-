...
def test_export_api_prevents_duplicate_downloads(monkeypatch):
    ...
    monkeypatch.setattr(web_ui._account_mgr, "accounts", {"a1": {"id": "a1"}})
    with tempfile.TemporaryDirectory() as directory:
...
def test_async_batch_retries_temporary_limit_after_cooldown():
    ...
    try:
        ...
        assert "每次等待 30 分钟后自动续建" in web_ui.UI_HTML
    finally:
        web_ui._account_mgr = original_manager
...
def test_ui_create_entry_and_scheduler_copy():
    ...
    assert "全局并发限制" in html
    assert "Account concurrency follows the creation protection setting" in html
